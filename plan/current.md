...
- [x] **PC 레이아웃 복구**: 합방 셀에 명시적 `grid-column` 지정을 추가하여 데스크탑 뷰에서 캐릭터 헤더를 침범하던 현상 수정.
- [x] **푸시 알림 HTML 태그 제거**: `stripHtml` 유틸을 도입하여 알림 본문에 `<div>` 등 HTML 요소가 포함되지 않도록 정제.
- [x] **모바일 그리드 안정화**: 모바일 뷰에서 모든 셀이 2번 컬럼에 고정되도록 CSS 강제 적용.
- [x] **모크 데이터(Mock) 클린업**: 로딩 전 노출되는 `MOCK_SCHEDULE`의 과거 데이터를 "스케쥴 준비중"으로 초기화하고 잘못된 아바타 경로 수정.
- [x] **FCM 일일 요약 알림(Cron) 스케줄 추가**: 매일 오전 9시(KST)에 동작되도록 `vercel.json` 및 `route.ts`에 Vercel CRON 연동 보안 처리.
